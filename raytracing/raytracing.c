...
{
    readThreadParams *args=arg;
	
        uint8_t *pixels = args->pixels;
	double* background_color = args->background;
	rectangular_node rectangulars  = args->rectangulars;
	sphere_node spheres = args->spheres;
	light_node lights = args->lights;
	const viewpoint *view = args ->view;
	int width = args->width;
	int height = args->height;
	int end_w = args->end_w;
	int end_h = args->end_h;
	int start_w = args->start_w;
	int start_h = args->start_h;
    point3 u, v, w, d;
